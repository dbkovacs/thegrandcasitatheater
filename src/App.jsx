            ...
            <div className="bg-brand-card p-8 rounded-2xl shadow-2xl border-2 border-yellow-300/20">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        ...
                        <input type="text" id="hostName" value={hostName} onChange={(e) => setHostName(e.target.value)} className="w-full bg-black/30 border-2 border-yellow-300/30 rounded-lg p-3 text-white focus:outline-none focus:border-brand-gold" />
                    </div>
                    <div>
                        <label htmlFor="movieTitle" className="block text-sm font-bold text-yellow-300/80 mb-2 font-cinzel">Movie Title</label>
                        <input type="text" id="movieTitle" value={movieTitle} onChange={(e) => setMovieTitle(e.target.value)} className="w-full bg-black/30 border-2 border-yellow-300/30 rounded-lg p-3 text-white focus:outline-none focus:border-brand-gold" />
                    </div>
                    <div>
                        <label htmlFor="eventDate" className="block text-sm font-bold text-yellow-300/80 mb-2 font-cinzel">Requested Date</label>
                        <input type="date" id="eventDate" value={eventDate} onChange={(e) => setEventDate(e.target.value)} className="w-full bg-black/30 border-2 border-yellow-300/30 rounded-lg p-3 text-white focus:outline-none focus:border-brand-gold" />
                    </div>
                    <div>
                    ...
                    <div className="pt-4">
                        <button type="submit" disabled={isSubmitting} className="w-full btn-velvet primary text-lg">
