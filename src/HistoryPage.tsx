...
const MovieHistoryCard: React.FC<{ movie: MovieNight }> = ({ movie }) => (
    <div className="bg-brand-card rounded-lg overflow-hidden border border-brand-gold/20 shadow-lg">
        <img src={movie.posterURL || `https://placehold.co/400x600/2a0000/fde047?text=${encodeURIComponent(movie.movieTitle)}`} alt={`${movie.movieTitle} Poster`} className="w-full h-auto object-cover aspect-[2/3]"/>
        <div className="p-4">
...
function HistoryPage() {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            ...
            const q = query(collection(db, "movieNights"), where("status", "==", "Approved"), where("showDate", "<", today), orderBy("showDate", "desc"));
            const querySnapshot = await getDocs(q);
            setPastMovies(querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as MovieNight)));
            setIsLoading(false);
        };
        fetchHistory();
    }, []);
